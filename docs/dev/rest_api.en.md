...
!!! info "hint"
    The API documentation has been integrated into the code by default and can be accessed through the following methods after deployment.
...
!!! tip "JumpServer API supports the following authentication methods"
    ```
    Session         You can use it directly after logging in session_id as authentication method
    Token           Get one-time Token，the Token Has expiry date, void after expiration
    Private Token   permanent Token
    Access Key      Yes Http Header Sign
    ```

    === "Session"
        When the user logs in after passing the page, jms_sessionid will exist in the cookie. When requesting, jms_sessionid will also be placed in the cookie.
        ...
        === "Java"
            ...
            public class HttpsClientTest {
                ...
                public static void https(String params, String token, String uri) throws IOException {
                    ...
                    // Ignore certificate authentication
    ...
    === "Private Token"
        ...
        ```
        private_token already exists and can be obtained directly
        ```python
        u.private_token
        ```
        Take PrivateToken: 937b38011acf499eb474e2fecb424ab3 as an example:
        ```sh
    ...
    === "Access Key"
        Create or obtain AccessKeyID AccessKeySecret in the API Key list of the web page
        === "Python"
            ```python
            # Python Example
        ...
        === "Java"
            ...
            public class JMSApiClient {
                ...
                public static void disableCertificateValidation(OkHttpClient.Builder builderClient) {
                    // Create a trust all certificates TrustManager
                    try {
                        ...
                        // create SSLContext，and associate trusting all certificates TrustManager
                        SSLContext sslContext = SSLContext.getInstance("TLS");
                        sslContext.init(null, trustAllCerts, new SecureRandom());
                        // create OkHttpClient.Builder，and set SSLContext builder
