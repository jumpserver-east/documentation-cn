...
2. 检查数据库状态（内置/外置），按照 [数据库故障](#database-failure) 的处理方式恢复数据库；
3. 启动相关依赖服务：
 ```bash
...
2. 参考《基本运维操作》中的磁盘空间管理章节清理过期录像、任务日志与旧备份文件；
...
3. 查看该组件日志定位具体报错（日志位于 `$VOLUME_DIR/<组件名称>/data/logs/` 目录，参考《组件日志查看》）；
