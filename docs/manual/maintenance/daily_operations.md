...
### 3.3 定时自动备份 { #scheduled-automatic-backups }
...
## 5 磁盘空间管理 { #disk-space-management }
...
2. **清理过期数据库备份**：参考 [3.3 定时自动备份](#scheduled-automatic-backups) 中的定时清理示例，按保留策略删除旧备份文件；
...
| 磁盘空间 | 每日 | `df -h` | 使用率低于 80%，超出时参考 [磁盘空间管理](#disk-space-management) 处理 |
...
| 核心日志 | 每周 | 查看 core 组件日志（参考《组件日志查看》） | 无持续出现的 ERROR 级别报错 |
